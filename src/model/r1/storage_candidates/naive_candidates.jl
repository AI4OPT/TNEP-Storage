...
function get_storage_candidates(data, filepath)
    ...
    for (key, value) in data["bus"]
        lat = round(value["lat"], digits=3)
        lon = round(value["lon"], digits=3)
...
function intersect_storage_candidates_original(data, no_upgrades_dir)
    ...
    println("Number of candidates: $(length(final_cand))")
    return final_cand
end
...
function save_to_json(cands, json_path)
    cands = collect(cands)
    open(json_path, "w") do file
        JSON.print(file, cands)
    end
end
