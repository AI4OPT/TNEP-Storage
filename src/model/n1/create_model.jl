...
function create_model_n1(data::Dict{String, Any}, optimizer)
    ...
    nonrenewable_generators = filter(g -> lowercase(data["gen"][g]["gen_type"]) ∉ ["solar", "wind"], keys(data["gen"]))
    @variable(model, pg[r in 1:R, g in 1:G, t in 1:T])

    for g in 1:G
        gen = data["gen"]["$g"]
        is_renewable = gen["gen_type"] ∈ ["solar", "wind"]
        if is_renewable
