...
fig = px.scatter_mapbox(
    ...
    lat='Lat',
    lon='Lon',
    size='wind_capacity',               # Size based on wind capacity
    color='wind_capacity',              # Color based on wind capacity
    color_continuous_scale='Tealgrn_r',    # Use viridis color scale
    center=dict(lat=31.5, lon=-99.5),    # Center the map
    zoom=4,                              # Set zoom level
    labels={'wind_capacity': 'Wind Capacity (100 MW)'},
...
# Save the map
fig.write_image(f"../../../{simdir}/visual/wind_cap.png")