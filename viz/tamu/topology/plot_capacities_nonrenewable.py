...
# Plot for Nonrenewable
fig = px.scatter_mapbox(
    df_nonrenewable,
    lat='Lat',
    lon='Lon',
    size='nonrenewable_capacity',               # Size based on nonrenewable capacity
    color='nonrenewable_capacity',              # Color based on nonrenewable capacity
    color_continuous_scale='Brwnyl_r',    # Use viridis color scale
    center=dict(lat=31.5, lon=-99.5),    # Center the map
    zoom=4,                              # Set zoom level
    labels={'nonrenewable_capacity': 'Nonrenewable Capacity (100 MW)'},
...
# Save the map
fig.write_image(f"../../../{simdir}/visual/nonrenewable_cap.png")