...
function export_energy_csv(simdir, model, data, rep_index, ptdf)
    ...
    if has_pf
        ...
        dis_values = value.(model[:dis])[rep_index:rep_index, :, :]
        
        num_branches = length(data["branch"])
        flows = zeros(num_branches, 1, num_hours)
        
        compute_flows!(flows, pg_values, ue_values, ch_values, dis_values, data, ptdf)
        pfs = flows[:, 1, :]
...
function export_flow(simdir, model, data, rep_index, ptdf)
